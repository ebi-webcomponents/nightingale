...
6. Create the file where the code of your custom element will be. In our example `src/nightingale-hello.ts`.
   ...
   2. The above example inherits from [LitElement](https://lit.dev/docs/api/LitElement/), while there is a base element for any Nightingale component `NightingaleElement`.
      It is recommended for any Nightinagle component to extend `NightingaleElement`.
      ...
      For instance, `NightingaleHello` needs to use margin nd highlight mixins, it is inherited as follows:
      ...
      In order to define attributes for the element, use Lit's `@property` decorator.
...
export const Hellow = Template.bind({});
Hello.args = {
  "name": "xxx",
};
```

The following command runs the storybook locally to test.
