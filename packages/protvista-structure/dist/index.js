(function (LiteMol) {
  ...
  LiteMol = LiteMol && LiteMol.hasOwnProperty('default') ? LiteMol['default'] : LiteMol;
  ...
  class ProtvistaStructure extends HTMLElement {
    ...
    connectedCallback() {
      ...
      this.loadUniProtEntry().then(entry => {
        ...
        this.selectMolecule(this._pdbEntries.filter(d => d.properties.method !== 'Model')[0].id);
      });
    }
    ...
    _planHighlight(withSelection = false) {
      ...
      if (this._plannedRender) return; // Set a flag and _planRender at the next frame
      ...
      requestAnimationFrame(() => {
        ...
        if (withSelection) {
          this._selectMoleculeWithinRange(this._highlightstart, this._highlightend).then(() => this.highlightChain());
        } else {
          this.highlightChain();
    ...
    loadStructureTable() {
      const html = `
            <table>
                ...
                <tbody>
                    ${this._pdbEntries.map(d => `
                        <tr id="entry_${d.id}" class="${d.properties.method === 'Model' ? 'pdb-row' : 'pdb-row-clickable'}" title="${d.properties.method === 'Model' ? 'No structure available for this model' : ''}">
                            <td>
                            <strong>${d.id}</strong><br/>
                            </td>
                            <td title="${d.properties.method}">${d.properties.method}</td>
                            <td>${d.properties.resolution}</td>
                            <td>${d.properties.chains.map(chain => `<div title="${chain.chains}">${chain.chain}</div>`).join('')}</td>
      ...
      this.querySelectorAll('.pdb-row-clickable').forEach(row => row.addEventListener('click', () => this.selectMolecule(row.id.replace('entry_', ''))));
    }
    ...
    loadLiteMol() {
      ...
      this.Context = Plugin.Components.Context; // Plugin.Components.Context.Log(this.Bootstrap.Components.LayoutRegion.Bottom, true);
      ...
      this._liteMol.context.highlight.addProvider(info => {// This is triggered on highlight
      });
    }
    ...
    translatePositions(start, end) {
      ...
      if ('string' === typeof this._highlightend || 'string' === typeof this._highlightstart) {
        return;
      }
      ...
      for (const mapping of this._selectedMolecule.mappings) {
        if (mapping.unp_end - mapping.unp_start === mapping.end.residue_number - mapping.start.residue_number) {
          if (start >= mapping.unp_start && end <= mapping.unp_end) {
            const offset = mapping.unp_start - mapping.start.residue_number; //TODO this is wrong because there are gaps in the PDB sequence
            ...
            this.addMessage(`Positions ${this._highlightstart}-${this._highlightend} not found in this structure`);
          }
        } else {
    ...
    highlightChain() {
      ...
      if (this.isResidueOnlyHighlight) {
        ...
        query = this.Query.sequence(translatedPos.entity.toString(), translatedPos.chain, {
          seqNumber: translatedPos.start
        }, {
          seqNumber: translatedPos.end
        });
      ...
      const action = this._liteMol.createTransform().add(visual, this.Transformer.Molecule.CreateSelectionFromQuery, {
        query: query,
        name: 'My name'
      }, {
