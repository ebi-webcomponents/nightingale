(function (ProtVistaUniProtEntryAdapter) {
  'use strict';
  ...
  class ProtvistaVariationAdapter extends ProtVistaUniProtEntryAdapter {
    ...
    parseEntry(data) {
      ...
      const variants = data.features; // ??? if (source && (source !== Constants.getUniProtSource())) {
      ...
      for (let variant of variants) {
        ...
        variant.xrefNames = variant.xrefs ? this.getSourceType(variant.xrefs, variant.sourceType) : []; //expected by tooltip
    ...
    formatTooltip(variant) {
      if (variant.description) variant.descriptionArray = this.getDescriptionsAsArray(variant.description);
      return `
            <table>
                ...
                ${variant.sourceType === 'UniProt' || variant.sourceType === 'mixed' ? this.getUniProtHTML(variant) : ''}
                ${variant.sourceType === 'large_scale_study' || variant.sourceType === 'mixed' ? this.getLSSHTML(variant) : ''}
            </table>
        `;
    }
    ...
    getDiseaseAssociations(associations) {
      return associations.map(association => `
            ...
            ${association.xrefs ? `<tr><td>Cross-references</td><td>${this.getXrefs(association.xrefs)}</td></tr>` : ''}
