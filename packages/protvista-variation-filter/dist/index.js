(function () {
     ...
     */
    const envCachesTemplates = (t => t() === t())(() => (s => s)``); // The first argument to JS template tags retain identity across multiple
    ...
    class Template {
      constructor(strings, svg = false) {
        ...
        this.element.innerHTML = this._getHtml(strings, svg); // Edge needs all 4 parameters present; IE11 needs 3rd parameter to be null

        const walker = document.createTreeWalker(this.element.content, 133
        ...
        while (walker.nextNode()) {
          ...
          /* Node.ELEMENT_NODE */
              ...
              for (let i = 0; i < attributes.length; i++) {
                ...
                if (attributeStrings.length > 1) {
                  ...
                  const attributeString = strings[partIndex]; // Trim the trailing literal value if this is an interpolation

                  const rawNameString = attributeString.substring(0, attributeString.length - attributeStrings[0].length); // Find the attribute name
          ...
          && node.nodeValue === textMarkerContent) {
            const parent = node.parentNode; // If we don't have a previous node add a marker node.
            ...
            this.parts.push(new TemplatePart('node', index++));
            nodesToRemove.push(node); // If we don't have a next node add a marker node.
      ...
      _getHtml(strings, svg) {
        ...
        for (let i = 0; i < l - 1; i++) {
          const s = strings[i];
          a.push(s); // We're in a text position if the previous string matches the
      ...
      class ProtvistaVariationFilter extends HTMLElement {
        ...
        toggleFilter(elt, filterName) {
          if (this._selectedFilters.filter(filt => filt.name === filterName).length > 0) {
            this._selectedFilters = this._selectedFilters.filter(filt => filt.name !== filterName);
            elt.classList.remove('active');
          } else {
            this._selectedFilters.push(filters.filter(filt => filt.name === filterName)[0]);
          ...
          this.dispatchEvent(new CustomEvent("change", {
            detail: {
              variantfilters: this._selectedFilters.map(d => d.name).toString()
            },
            bubbles: true,
            cancelable: true
          }));
