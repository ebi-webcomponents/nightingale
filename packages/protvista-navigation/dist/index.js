var wc = (function () {
  ...
  class ProtVistaNavigation extends HTMLElement {
    ...
    _createNavRuler() {
      this._x = linear$4().range([padding.left, this.width - padding.right]);

      this._x.domain([1, this._length]);

      this._svg = select$2(this).append('div').attr('class', '').append('svg').attr('id', '').attr('width', this.width).attr('height', height);
      ...
      this._viewport = brushX().extent([[padding.left, 0], [this.width - padding.right, height * 0.51]]).on("brush", () => {
        if (event$2.selection) {
          this._displaystart = format("d")(this._x.invert(event$2.selection[0]));
          this._displayend = format("d")(this._x.invert(event$2.selection[1]));
          if (!this.dontDispatch) this.dispatchEvent(new CustomEvent("change", {
            detail: {
              displayend: this._displayend,
              displaystart: this._displaystart,
              extra: {
                transform: event$2.transform
              }
            },
            bubbles: true,
            cancelable: true
