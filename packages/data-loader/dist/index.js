var wc = (function () {
    ...
    static async fetch(source) {
      ...
      if (typeof cached !== 'undefined') {
        ...
        if (source.src) {
          ...
          const response = window.fetch(source.src, {
            headers
          });
  ...
  const getSourceData = (...children) => children.filter(child => child.matches('source[src], script[type="application/json"]'));
  ...
  class DataLoader extends HTMLElement {
    ...
    async fetch() {
      ...
      const sources = getSourceData(...this.children); // if nothing there, bails
      ...
      let errors = [];
      let detail; // go over all the potential sources to try to load data from it

      for (const source of sources) {
        try {
          detail = await RequestManager.fetch(source);
          detail.srcElement = source;
          detail.src = source.src; // if we're here, we have data, go out of the loop
      ...
      if (typeof this.selector === 'string') {
        this._data = get(detail.payload, this.selector);
      } else {
