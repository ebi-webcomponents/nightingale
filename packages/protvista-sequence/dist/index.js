var wc = (function (d3,ProtvistaZoomable) {
  'use strict';
  ...
  class ProtVistaSequence extends ProtvistaZoomable {
    ...
    set data(data) {
      ...
      if (this.sequence) this._createSequence();
    }

    _createSequence() {
      super.svg = d3.select(this).append('div').attr('class', '').append('svg').attr('id', '').attr('width', this.width).attr('height', height);
    ...
    refresh() {
      if (this.axis) {
        ...
        this.xAxis = d3.axisBottom(this.xScale).tickFormat(d => Number.isInteger(d) ? d : '');
        ...
        this.bases = this.seq_g.selectAll('text.base').data(this.sequence.split(''));
        this.bases.enter().append('text').attr('class', 'base').attr('text-anchor', 'middle').attr('x', (b, i) => this.xScale(i + 1) + half).text(d => d).attr('style', 'font-family:monospace');
        this.bases.attr('x', (b, i) => this.xScale(i + 1) + half);
        this.background = this.seq_bg.selectAll('rect.base_bg').data(this.sequence.split(''));
        this.background.enter().append('rect').attr('class', 'base_bg').attr('fill', (d, i) => i % 2 ? '#ccc' : '#eee').attr('height', height).attr('width', ftWidth).attr('x', (b, i) => this.xScale(i + 1));
        this.background.attr('width', ftWidth).attr('x', (b, i) => this.xScale(i + 1));
        const space = super.xScale(2) - super.xScale(1) - this.seq_g.select('text.base').node().getBBox().width * 0.8;
        ...
        if (Number.isInteger(this._highlightstart) && Number.isInteger(this._highlightend)) {
          this.highlighted.attr('x', super.xScale(this._highlightstart)).style('opacity', 0.3).attr('width', ftWidth * (this._highlightend - this._highlightstart + 1));
        } else {
