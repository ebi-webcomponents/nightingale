...
var addToTree = function addToTree(nodes, items, parent) {
    if (items.length <= 0) {
        return;
    }
    ...
    try {
        for (var _iterator = nodes[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
            ...
            if (_node.name === items[0]) {
                addToTree(_node.children, items.slice(1), _node);
                return;
            }
        }
    } catch (err) {
...
function addStringItem(stringItem, tree) {
    var splitItems = stringItem.split(', ');
...
var subcellulartreeMenu = [];
var diseases = {};

function load(accession) {
    return fetch('https://www.ebi.ac.uk/proteins/api/proteins/interaction/' + accession + '.json').then(function (resp) {
...
function process(data) {
    ...
    try {
        for (var _iterator = data[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
            ...
            try {
                for (var _iterator2 = element.interactions[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
            ...
            if (element.subcellularLocations) {
                ...
                try {
                    for (var _iterator3 = element.subcellularLocations[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
                        ...
                        try {
                            for (var _iterator4 = location.locations[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
                                var actualLocation = _step4.value;

                                addStringItem(actualLocation.location.value, subcellulartreeMenu);
                                var locationSplit = actualLocation.location.value.split(', ');
            ...
            if (element.diseases) {
                ...
                try {
                    for (var _iterator5 = element.diseases[Symbol.iterator](), _step5; !(_iteratorNormalCompletion5 = (_step5 = _iterator5.next()).done); _iteratorNormalCompletion5 = true) {
                        var disease = _step5.value;
...
function render(_ref) {
    var _ref$el = _ref.el,
        el = _ref$el === undefined ? required('el') : _ref$el,
        _ref$accession = _ref.accession,
        accession = _ref$accession === undefined ? 'P05067' : _ref$accession;
    ...
    load(accession).then(function (data) {
...
function formatSubcellularLocationInfo(data) {
    if (data) {
        var formatedString = '<ul class="tree-list">';
        var tree = [];
        var _iteratorNormalCompletion2 = true;
        var _didIteratorError2 = false;
        var _iteratorError2 = undefined;
        ...
        try {
            for (var _iterator2 = data[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
                var interactionType = _step2.value;
                var _iteratorNormalCompletion3 = true;
                var _didIteratorError3 = false;
                var _iteratorError3 = undefined;

                try {
                    for (var _iterator3 = interactionType.locations[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
                        var location = _step3.value;
        ...
        traverseTree(tree, function (d) {
            return formatedString += '<li style="margin-left:' + d.depth + 'em">' + d.name + '</li>';
        });
...
function draw(el, accession, data) {
    ...
    var tooltip = select(el).append("div").attr("class", "interaction-tooltip").attr("display", "none").style("opacity", 0);
    tooltip.append('span').attr('class', 'close-interaction-tooltip').text('Close ✖').on('click', closeTooltip);
    tooltip.append('div').attr('class', 'tooltip-content');

    select(el).append("p").attr("class", "interaction-title").text(accession + ' has binary interactions with ' + (nodes.length - 1) + ' proteins');
    ...
    var x = band().rangeRound([0, width]),
        intensity = linear().range([0.2, 1]);

    var svg = select(el).append("svg").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).attr("class", "interaction-viewer").append("g").attr("class", "interaction-viewer-group").attr("transform", "translate(" + margin.left + "," + margin.top + ")");

    x.domain(nodes.map(function (entry) {
    ...
    var row = svg.selectAll(".interaction-row").data(nodes).enter().append("g").attr("class", "interaction-row").attr("transform", function (d) {
        return 'translate(0,' + x(d.accession) + ')';
    }).each(processRow);

    row.append("rect").attr("x", -margin.left).attr("width", margin.left).attr("height", x.bandwidth()).attr("class", "text-highlight");

    // left axis text
    row.append("text").attr("y", x.bandwidth() / 2).attr("dy", ".32em").attr("text-anchor", "end").text(function (d, i) {
        return nodes[i].name;
    }).attr('class', function (d, i) {
        return nodes[i].accession === accession ? "main-accession" : "";
    });

    var column = svg.selectAll(".column").data(nodes).enter().append("g").attr("class", "column").attr("transform", function (d) {
        return 'translate(' + x(d.accession) + ', 0)rotate(-90)';
    });

    column.append("rect").attr("x", 6).attr("width", margin.top).attr("height", x.bandwidth()).attr("class", "text-highlight");

    // top axis text
    column.append("text").attr("x", 6).attr("y", x.bandwidth() / 2).attr("dy", ".32em").attr("text-anchor", "start").text(function (d, i) {
        return nodes[i].name;
    }).attr('class', function (d, i) {
        return nodes[i].accession === accession ? "main-accession" : "";
    });
    ...
    svg.append("polyline").attr("points", points).attr("class", "hidden-side").attr("transform", function (d) {
        return 'translate(' + x(nodes[1].accession) + ', 0)';
    });
    ...
    function processRow(row) {
        ...
        var cell = select(this).selectAll(".cell").data(row.interactions);

        var circle = cell.enter().append("circle");

        circle.attr("class", "cell").attr("cx", function (d) {
            return x(d.id) + x.bandwidth() / 2;
        }).attr("cy", function (d) {
            return x.bandwidth() / 2;
        }).attr("r", x.bandwidth() / 3).style("fill-opacity", function (d) {
            return intensity(d.experiments);
        }).style("display", function (d) {
            //Only show left half of graph
            return x(row.accession) < x(d.id) ? "none" : "";
        }).on("click", mouseclick).on("mouseover", mouseover).on("mouseout", mouseout);
    ...
    function mouseover(p) {
        ...
        selectAll(".interaction-row").classed("active", function (d) {
            return d.accession === p.id;
        });
        // selectAll(".column").classed("active", d => d.accession === p.id);

        selectAll('.interaction-viewer-group').append('line').attr('class', 'active-row').attr('style', 'opacity:0').attr('x1', 0).attr('y1', x(p.source) + x.bandwidth() / 2).attr('x2', x(p.id)).attr('y2', x(p.source) + x.bandwidth() / 2);

        selectAll('.interaction-viewer-group').append('line').attr('class', 'active-row').attr('style', 'opacity:0').attr('x1', x(p.id) + x.bandwidth() / 2).attr('y1', 0).attr('x2', x(p.id) + x.bandwidth() / 2).attr('y2', x(p.source));
    }

    function mouseclick(p) {
        populateTooltip(selectAll('.tooltip-content'), p);
        tooltip.style("opacity", 0.9).style("display", "inline").style("left", mouse(el)[0] + 10 + "px").style("top", mouse(el)[1] - 15 + "px");
    }
    ...
    function populateTooltip(element, data) {
        ...
        var source = nodes.find(function (d) {
            return d.accession === data.source;
        });
        var target = nodes.find(function (d) {
            return d.accession === data.id;
        });

        element.append('h3').text('Interaction');
        element.append('p').append('a').attr('href', getIntactLink(data.interactor1, data.interactor2)).attr('target', '_blank').text('Confirmed by ' + data.experiments + ' experiment(s)');
        ...
        intactRow.append('td').attr('colspan', 2).append('a').attr('href', getIntactLink(data.interactor1, data.interactor2)).attr('target', '_blank').text(data.interactor1 + ';' + data.interactor2);
    }
    ...
    function closeTooltip() {
        selectAll('.interaction-tooltip').style("opacity", 0).style("display", "none");
    }
}
...
function hasFilterMatch(source, target, filters) {
    ...
    return intersection(source.filterTerms, filters.map(function (item) {
        return item['name'];
    })).length > 0 || intersection(target.filterTerms, filters.map(function (item) {
        return item['name'];
    })).length > 0;
}
...
function filterData() {
    ...
    var visibleAccessions = [];
    selectAll('.cell').attr('opacity', function (d) {
        var source = getNodeByAccession(d.source);
        var target = getNodeByAccession(d.id);
        var visible = hasFilterMatch(source, target, activeFilters);
        if (visible) {
...
function toggleFilterVisibility() {
    var id = '#' + select(this).attr('data-toggle');
    var visibility = select(id).style('visibility');
...
function createFilter(el, filtersToAdd) {
    ...
    var container = select(el).append("div").attr("class", "interaction-filter-container");
    ...
    var _loop = function _loop(filter) {
        if (filter.items.length > 0) {
            filterContainer = container.append("div").attr("class", "interaction-filter");

            filterContainer.append("a").text(filter.label).attr("class", "button dropdown").attr("data-toggle", 'iv_' + filter.name).on("click", toggleFilterVisibility);
            if (filter.type === 'tree') {
                ul = filterContainer.append("ul").attr("id", 'iv_' + filter.name).attr("class", "dropdown-pane");

                traverseTree(filter.items, function (d) {
                    d.type = filter.name;
                    filters.push(d);
                    ul.datum(d).append('li').style("padding-left", d.depth + "em").attr("id", function (d) {
                ...
                filterContainer.append("ul").attr("id", 'iv_' + filter.name).attr("class", "dropdown-pane").selectAll('li').data(filter.items).enter().append('li').attr('id', function (d) {
                    return getNameAsHTMLId(d.name);
                }).text(function (d) {
    ...
    container.append("button").attr('class', 'iv_reset').text("Reset filters").on('click', function (d) {
        resetAllFilters();
        return false;
    });
}
...
var loadComponent = function loadComponent() {
    var InteractionViewer = function (_HTMLElement) {
        inherits(InteractionViewer, _HTMLElement);

        function InteractionViewer() {
            classCallCheck(this, InteractionViewer);

            var _this = possibleConstructorReturn(this, (InteractionViewer.__proto__ || Object.getPrototypeOf(InteractionViewer)).call(this));

            _this._accession = _this.getAttribute('accession');
            return _this;
        }

        createClass(InteractionViewer, [{
