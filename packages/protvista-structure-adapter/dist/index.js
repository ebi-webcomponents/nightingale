var wc = (function (ProtVistaUniProtEntryAdapter) {
  ...
  class ParserHelper {
    ...
    static getDescription(properties) {
      let description = '';
      forEach$1(properties, (property, key) => {
  ...
  class StructureDataParser {
    ...
    _validateEntry(data) {
      ...
      const structures = filter(data.dbReferences, reference => {
    ...
    _getAllFeatureStructures(data) {
      ...
      allFeatureStructures = map(structures, structure => {
        const beginEnd = structure.properties.chains ? ParserHelper.getBeginEnd(structure.properties.chains) : {
          start: 0,
          end: 0
        };
        return {
    ...
    _getOverlapping(ftStructure, startEnd) {
      const overlapping = remove(this._pdbFeatures, ftCoverage => {
        if (ftCoverage.start <= ftStructure.start && ftStructure.start <= ftCoverage.end || ftCoverage.start <= ftStructure.end && ftStructure.end <= ftCoverage.end || ftStructure.start <= ftCoverage.end && ftCoverage.end <= ftStructure.end) {
    ...
    _parseValidEntry(data) {
      ...
      forEach(allFeatureStructures, ftStructure => {
        const startEnd = {
  ...
  /*jslint node: true */
  class ProtVistaStructureAdapter extends ProtVistaUniProtEntryAdapter {
    constructor() {
      super();
  ...
  return ProtVistaStructureAdapter;
