var wc = (function (exports,ProtvistaZoomable) {
  'use strict';
  ...
  class FeatureShape {
    ...
    _diamond() {
      ...
      const shape = 'M' + m + ',0' + 'L' + (m + centerx) + ',' + centerx + 'L' + m + ',' + symbolSize + 'L' + (m - centerx) + ',' + centerx;
    ...
    _chevron() {
      const m = this._ftWidth / 2;
      const centerx = symbolSize / 2;
      const shape = 'M' + m + ',' + centerx + 'L' + (centerx + m) + ',0' + 'L' + (centerx + m) + ',' + centerx + 'L' + m + ',' + symbolSize + 'L' + (-centerx + m) + ',' + centerx + 'L' + (-centerx + m) + ',0';
    ...
    _circle() {
      ...
      const shape = "M" + m + ",0" + "A" + r + "," + r + " 0 1,1 " + m + "," + symbolSize + "A" + r + "," + r + " 0 1,1 " + m + ",0";
    ...
    _arrow() {
      ...
      const m = this._ftWidth / 2;
      const shape = 'M' + m + ',0' + 'L' + (-step + m) + ',0' + 'L' + (-5 * step + m) + ',' + 4 * step + 'L' + (-step + m) + ',' + this._ftHeight + 'L' + m + ',' + this._ftHeight + 'L' + (4 * step + m) + ',' + 4 * step;
    ...
    _doubleBar() {
      const m = this._ftWidth / 2;
      const centerx = symbolSize / 2;
      const shape = 'M' + m + ',0' + 'L' + (-centerx + m) + ',' + symbolSize + 'L' + m + ',' + symbolSize + 'L' + (centerx + m) + ',0';
      return this._ftLength !== 1 ? shape + 'L' + m + ',0' + this._getMiddleLine(symbolSize / 2, this._ftWidth) + 'Z' : shape + 'Z';
    }
  ...
  function castFunction(value) {
    return typeof value == 'function' ? value : identity$d;
  }
  ...
  class Row {
    ...
    _featureOverlap(feature, d, ftEnd, dEnd) {
      const featureBeginOverlap = Number(feature.start) >= Number(d.start) && Number(feature.start) <= Number(dEnd);
    ...
    _addAbsoluteLimits(feature) {
      const limits = feature.locations.reduce((acc, e) => acc.concat(e.fragments), []).reduce((acc, e) => ({
    ...
    containsOverlap(feature) {
      ...
      return some(this._rowFeatures, d => {
  ...
  class NonOverlappingLayout extends DefaultLayout {
    ...
    init(features) {
      ...
      forEach(this._features, feature => {
        const added = some(this._rows, row => {
    ...
    getFeatureYPos(feature) {
      let yPos;
      const yOffset = this._layoutHeight / this._rows.length > this._minHeight ? (this._layoutHeight - this._rows.length * this._minHeight) / 2 : 0;

      forEach(this._rows, (row, i) => {
        forEach(row._rowFeatures, currFeature => {
  ...
  class ProtVistaTrack extends ProtvistaZoomable {
    ...
    connectedCallback() {
      ...
      this.addEventListener('load', e => {
        if (includes(this.children, e.target)) {
    ...
    _createTrack() {
      ...
      select$2(this).selectAll('*').remove();
      select$2(this).html('');
      this.svg = select$2(this).append('div').style('line-height', 0).append('svg').attr('width', this.width).attr('height', this._height);
    ...
    _createFeatures() {
      ...
      this.locations = this.featuresG.enter().append('g').attr('class', 'feature-group').attr('id', d => `g_${d.accession}`).selectAll('g.location-group').data(d => d.locations.map(loc => Object.assign({}, loc, {
        feature: d
      }))) // .data(d => d.locations.map((loc) => ({ feature: d, ...l })))
      .enter().append('g').attr('class', 'location-group');
      this.features = this.locations.selectAll('g.fragment-group').data(d => d.fragments.map(loc => Object.assign({}, loc, {
        feature: d.feature
      }))) // .data(d => d.fragments.map(({ ...l }) => ({ feature: d.feature, ...l })))
      .enter().append('path').attr('class', 'feature').attr('tooltip-trigger', 'true').attr('d', f => this._featureShape.getFeatureShape(this.xScale(2) - this.xScale(1), this._layoutObj.getFeatureHeight(f), f.end ? f.end - f.start + 1 : 1, this._getShape(f.feature))).attr('transform', f => 'translate(' + this.xScale(f.start) + ',' + (margin.top + this._layoutObj.getFeatureYPos(f.feature)) + ')').attr('fill', f => this._getFeatureColor(f.feature)).attr('stroke', f => this._getFeatureColor(f.feature)).on('mouseover', f => {
        var self = this;
        var e = event$2;
        ...
        if (this._tooltipEvent === 'click') {
          this.createTooltip(event$2, d, true);
        }
      });
    }
    ...
    createTooltip(e, d, closeable = false) {
      ...
      const tooltipPosition = select$2(tooltip).node().getBoundingClientRect();
    ...
    refresh() {
      if (this.xScale && this.seq_g) {
        this.features = this.seq_g.selectAll('path.feature').data(this._data.reduce((acc, f) => acc.concat(f.locations.reduce((acc2, e) => acc2.concat(e.fragments.map(loc => Object.assign({}, loc, {
          feature: f
        }))), [])), []));
        this.features.attr('d', f => this._featureShape.getFeatureShape(this.xScale(2) - this.xScale(1), this._layoutObj.getFeatureHeight(f), f.end ? f.end - f.start + 1 : 1, this._getShape(f.feature))).attr('transform', f => 'translate(' + this.xScale(f.start) + ',' + (margin.top + this._layoutObj.getFeatureYPos(f.feature)) + ')');
    ...
    _updateHighlight() {
      if (Number.isInteger(this._highlightstart) && Number.isInteger(this._highlightend)) {
        this.highlighted.attr('x', this.xScale(this._highlightstart)).style('opacity', 0.3).attr('width', this.xScale(this._highlightend - this._highlightstart + 1) - this.xScale(0));
      } else {
